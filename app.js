...
const CUSTOMER_COMPREHENSIVE_CHAPTER_SUMMARIES = [
  ...
  {
    pattern: /^三、到访与分诊/,
    summary: '客户已到店，命中 1 条 2026-01-05 16:57:30 的杭州颜术新芽医疗美容诊所到访记录，状态为 Status=1、TreatmentStatus=1、ConsultStatus=0、IsTriage=0。该到访未关联预约，未记录咨询师和护士；到访登记医生为祝媛园，医生病历医生为杨慧。TreatmentBeginTime=2026-01-05 16:58:00 表明已进入就诊页面，但分诊未完成或未记录完成，并可推断为新客到访。',
  },
  {
    pattern: /^四、新客基础信息/,
    summary: '新客基础信息显示客户约 52 岁，来源为客户推荐/老带新，所属诊所为杭州颜术新芽医疗美容诊所。医生链路仍为客户主表医生杨慧、病历医生杨慧、到访登记医生祝媛园，初诊时间为 2026-01-05。结构化皮肤评分未命中，但医生病历有皮肤问题文本；基础健康和生活习惯中 Smoke=0、Pregnancy=0、FeedBaby=0，客户主表相关健康字段未填，病历否认药物过敏、传染病、手术外伤和遗传性疾病家族史。',
  },
  {
...
const CUSTOMER_SUMMARY_HIGHLIGHT_RE = /(陈喜生|RFM\s*[A-Z]|粉星|黄褐斑|日光性黑子|面部皮肤松弛|皮秒激光|美塑疗法|CSKIN|企微|SUCCESS|已付款|无欠费|无余额|客户推荐\/老带新|杭州颜术新芽医疗美容诊所|杨慧|陈曦|祝媛园|未命中|未找到|无法填写|待确认|1 月后复诊|划扣|消耗|客诉|理赔|退款|积分|推荐关系|医生病历|补注射|超皮秒|消炎水光|大师霜|神经酰胺霜|\d{4}-\d{2}-\d{2}(?: \d{2}:\d{2}:\d{2})?|\d+(?:\.\d{2})?)/g;
...
function renderChenLifecycleAgentAnalysis(analysis) {
  ...
  const contextCards = Array.isArray(analysis.contextCards) && analysis.contextCards.length
    ...
    : [
        ...
        { label: '机构与医生', text: '门店为杭州颜术新芽医疗美容诊所；客户主表所属医生和病历医生为杨慧，到访登记医生为祝媛园。' },
      ];
